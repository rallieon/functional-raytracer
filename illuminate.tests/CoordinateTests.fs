...
module CoordinateTests = 
    let getRasterPlane =
        {width = 640; height = 480; fov = 90; shapes = List.empty; lights = List.empty; camera = {cameraOrigin = vector [0.; 0.; 0.]}; debugi = None; debugj = None; debug = false}
    ...
    let ``can convert middle screen coordinate to world coordinate`` () =
        ...
        let rasterPlane = getRasterPlane
        let conversion = mapScreenCoordinateToWorldCoodinate screenCoordinate rasterPlane
        Assert.Equal(0.00, conversion.[0], 2)
        Assert.Equal(0.00, conversion.[1], 2)
